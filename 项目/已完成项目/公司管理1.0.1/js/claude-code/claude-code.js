...
const ClaudeCode = {
    ...
    _renderAccounts(active, disabled) {
        ...
        const renderCard = (acc) => {
            ...
            if (this.usageCache[acc.id]) {
                const { dailyCost, dailyLimit } = this.usageCache[acc.id];
    ...
    async _loadUsage(acc) {
        ...
        try {
            ...
            `;
            this.usageCache[acc.id] = { dailyCost, dailyLimit };
    ...
    async createAccount() {
        ...
        try {
            const resp = await fetch(this._apiUrl(), {
                method: 'POST',
                headers: this._headers(),
                body: JSON.stringify({ name, base_url, auth_token, query_url, is_active: 'true' })
            });
            const result = await resp.json();
            if (result.error) throw new Error(result.error);
