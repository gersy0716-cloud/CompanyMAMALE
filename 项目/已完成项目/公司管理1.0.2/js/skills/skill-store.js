...
const Skills = {
    ...
    async showDetail(id) {
        ...
        // 仅限发布者本人可见操作菜单
        const isOwner = Auth.currentUser && String(Auth.currentUser.name) === String(skill.user_id);
        ...
        // 构造统一的 XHS 风格自定义 Header
        const headerHTML = `
            <div class="xhs-modal-header-custom">
                <div class="user-avatar xhs-header-avatar" style="background:${color}">
                    ${initial}
                </div>
                <div style="display: flex; flex-direction: column; justify-content: center; flex: 1;">
                    <span class="xhs-header-name" style="line-height:1.2;">${Utils.escapeHTML(name)}</span>
                    <span style="font-size:11px; color:#94a3b8; margin-top:2px;">分享于 ${Utils.formatDate(skill.created_at)}</span>
                </div>
                <div class="xhs-header-more">
                    ${isOwner ? `
                        <button class="xhs-more-btn" onclick="Skills.toggleMoreActions(event)">
        ...
        const bodyHTML = `
            <div class="xhs-detail-container">
                <div class="xhs-content-section" style="padding-top: 24px;">
                    <h1 class="xhs-title" style="margin-bottom: 12px;">${Utils.escapeHTML(skill.name)}</h1>
                    
                    <div class="skill-tags" style="margin-bottom: 20px; border: none; padding: 0; background: none;">
                        ${(skill.tags || []).map(tag => `<span class="skill-tag" style="background:#f1f5f9; color:#64748b; padding:4px 10px; border-radius:6px; font-size:12px; font-weight:500;">${Utils.escapeHTML(tag)}</span>`).join('')}
                    </div>

                    <div class="xhs-body-text" style="color: #64748b; margin-bottom: 24px; font-style: italic; border-left: 3px solid #e2e8f0; padding-left: 12px;">
                        ${Utils.escapeHTML(skill.summary)}
                    </div>
                    ...
                    <div style="margin-top: 24px;">
                        ...
                        <div class="xhs-body-text markdown-body" style="line-height: 1.8;">
                            ${Utils.escapeHTML(skill.detail).replace(/\n/g, '<br>')}
                        </div>
                    </div>
                    ` : ''}

                    <div id="skill-attachments" style="margin-top: 32px;">
                        <h4 style="font-size: 15px; font-weight: 700; color: #1e293b; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
                    ...
                    <div class="xhs-post-time-aligned">${Utils.formatDate(skill.created_at)}</div>
                </div>
            </div>
        `;
