...
async function checkAll() {
    ...
    try {
        ...
        if (basesRes.ok) {
            ...
            console.log(`Base listing skipped (status: ${basesRes.status}). Proceeding to table checks...`);
        }
    } catch (e) {
        console.log('Base listing timeout or error, skipping... Proceeding to table checks...');
    }
    ...
    const tables = [
        ...
        { id: Config.TABLES.DISCUSSIONS, name: 'discussions' },
        { id: Config.TABLES.TASKS, name: 'tasks' },
        { id: Config.TABLES.VERSIONS, name: 'versions' }
    ];
