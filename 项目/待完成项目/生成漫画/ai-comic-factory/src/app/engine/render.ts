...
// TuZi (兔子通道) config
const tuziApiBaseUrl = `${process.env.TUZI_API_BASE_URL || "https://3w-api.mamale.vip/api/app"}`
const tuziApiModel = `${process.env.TUZI_API_MODEL || "gemini-3.1-flash-image-preview"}`
const tuziApiToken = `${process.env.TUZI_API_TOKEN || ""}`
...
export async function newRender({
  ...
  try {
    if (renderingEngine === "TUZI") {
      // ===== 兔子通道 (TuZi) 异步图片生成 =====
      if (!tuziApiToken) {
        throw new Error(`invalid tuziApiToken, you need to configure TUZI_API_TOKEN`)
      }

      const createTaskUrl = `${tuziApiBaseUrl}/tuZi/asyncImageCreateMy`

      console.log(`[TuZi] Creating async task with model: ${tuziApiModel}`)

      const createRes = await fetch(createTaskUrl, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${tuziApiToken}`
        },
        body: JSON.stringify({
          model: tuziApiModel,
          prompt,
          size: width > height ? "16:9" : width < height ? "9:16" : "1:1"
        }),
        cache: "no-store"
      })
      ...
      if (!createRes.ok) {
        ...
        throw new Error(`TuZi task creation failed (${createRes.status}): ${errorText}`)
      }

      const createResult = await createRes.json()
      const taskId = createResult.taskId
      if (!taskId) {
        throw new Error(`TuZi task created but no taskId returned: ${JSON.stringify(createResult)}`)
      }

      console.log(`[TuZi] Task created: ${taskId}, polling...`)

      // Poll for completion
      const queryUrl = `${tuziApiBaseUrl}/tuZi/asyncImageQueryMy/${taskId}`
      let imageUrl = ""
      const maxPolls = 60
      ...
      for (let i = 0; i < maxPolls; i++) {
        ...
        const queryRes = await fetch(queryUrl, {
          method: "GET",
          headers: { "Authorization": `Bearer ${tuziApiToken}` },
          cache: "no-store"
        })

        if (!queryRes.ok) {
          throw new Error(`TuZi query failed (${queryRes.status})`)
        }

        const queryResult = await queryRes.json()
        const { status, video_url, isSucess } = queryResult

        console.log(`[TuZi] Poll ${i + 1}: status=${status}, success=${isSucess}`)

        if (status === "completed" && isSucess && video_url) {
          imageUrl = video_url
          break
        } else if (status === "failed" || (status === "completed" && !isSucess)) {
          throw new Error("TuZi image generation failed")
        }
      }

      if (!imageUrl) {
        throw new Error("TuZi image generation timed out")
      }

      console.log(`[TuZi] Image ready: ${imageUrl}`)
