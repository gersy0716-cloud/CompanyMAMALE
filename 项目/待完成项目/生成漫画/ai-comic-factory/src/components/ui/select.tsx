...
>(({ className, children, ...props }, ref) => (
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown suppressHydrationWarning className="h-4 w-4 opacity-50" />
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn("py-3 pl-10 pr-4 text-lg font-bold text-[var(--text-muted)]", className)}
    {...props}
  />
))
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-md py-4 pl-10 pr-4 text-lg outline-none focus:bg-[var(--primary)] focus:text-white data-[disabled]:pointer-events-none data-[disabled]:opacity-50 dark:focus:bg-stone-800 dark:focus:text-stone-50 transition-colors",
      className
    )}
    {...props}
  >
    <span className="absolute left-3 flex h-5 w-5 items-center justify-center">
      <SelectPrimitive.ItemIndicator>
        <Check suppressHydrationWarning className="h-6 w-6" />
      </SelectPrimitive.ItemIndicator>
    </span>
