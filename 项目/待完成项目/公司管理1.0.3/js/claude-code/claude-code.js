...
const ClaudeCode = {
    ...
    _parseRemark(remark) {
        ...
        return remark.split(',').map(s => s.trim()).filter(Boolean);
    },

    /**
     * 将用户名数组序列化为 remark 字符串
     */
    _serializeRemark(names) {
        return names.join(', ');
    },
    ...
    _renderAccounts(active, disabled) {
        ...
        const renderCard = (acc) => {
            ...
            // 正在使用状态
            // 解析 remark 为多用户列表
            const remarkUsers = this._parseRemark(acc.remark);
            const remarkDisplay = remarkUsers.length > 0 ? remarkUsers.join('、') : '暂无';
            const statusHTML = `
                <div class="cc-user-status ${remarkUsers.length > 0 ? '' : 'empty'}">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                    <span class="cc-status-text">${this._isActive(acc) ? ('使用中：' + remarkDisplay + (remarkUsers.length > 1 ? ' (' + remarkUsers.length + '人)' : '')) : '已禁用'}</span>
                </div>
            `;
            ...
            const isCheckedIn = remarkUsers.includes(userName);
    ...
    async switchTo(accountId) {
        ...
        // 2. 自动登记当前使用人 (服务器端记录，支持多人签到)
        const user = Auth.currentUser;
        if (user) {
            const userName = user.real_name || user.name || '未知用户';
            try {
                // 将当前用户追加到该账号的 remark（不影响其他已签到的人）
                const currentUsers = this._parseRemark(acc.remark);
                if (!currentUsers.includes(userName)) {
                    currentUsers.push(userName);
                    acc.remark = this._serializeRemark(currentUsers);

                    // 提交到服务器
                    await fetch(this._apiUrl(accountId), {
                        method: 'PATCH',
                        headers: this._headers(),
                        body: JSON.stringify({ remark: acc.remark })
                    });
                }
    ...
    async checkIn(accountId) {
        ...
        if (currentUsers.includes(userName)) {
            // 签退：从列表中移除当前用户
            try {
                const updatedUsers = currentUsers.filter(n => n !== userName);
                acc.remark = this._serializeRemark(updatedUsers);
