...
打包过程由 `electron-builder` 驱动，输出文件为 `Mamale_Management_V1.0.3_Setup.exe`。
...
### ⌨️ Claude Code 一键切换 (V1.0.3 改版)

1. 应用通过 `IPC` 桥接直接连接本地文件系统。
2. 切换账号时，物理写入 `~/.claude/settings.json`。
3. **多人签到**：`remark` 字段支持逗号分隔的多用户列表（如 `罗文彬, 张三`），签到追加、签退移除，不影响其他已签到的人。
4. **强制刷新**：切换或签到后会自动触发 `loadAccounts(true)`，解决 UI 状态不同步的问题。
